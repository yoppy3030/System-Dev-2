...
async function loadCultureNavbar() {
  try {
    ...
    if (window.sharedNavLoaded) {
      console.log('[load-navbar-culture.js] shared-nav.js already loaded, reinitializing');
      if (typeof initializeDropdowns === 'function') {
        console.log('[load-navbar-culture.js] Calling initializeDropdowns');
        initializeDropdowns();
      } else {
        console.warn('[load-navbar-culture.js] initializeDropdowns is not a function');
      }
      if (typeof initializeLanguageSelector === 'function') {
        console.log('[load-navbar-culture.js] Calling initializeLanguageSelector');
        initializeLanguageSelector();
      } else {
        console.warn('[load-navbar-culture.js] initializeLanguageSelector is not a function');
      }
      return;
    }
