...
// 翻訳データの読み込み
Promise.all([
    fetch('./js/translations/travelers-ja.json').then(response => response.json()),
    fetch('./js/translations/travelers-zh.json').then(response => response.json())
])
.then(([jaData, zhData]) => {
    translations = jaData.translations;
    translationsZh = zhData.translations.zh;
